...
let appropriateTimeForNextNotification =
    (today: float, notifFrequency: array(option(int))) => {
  open Js.Date;
  ...
  let supposedMaxLimitDate =
    supposedDate->getTime -. 1000. *. 60. *. minutesGapToAvoidTooCloseNotif;
  let adjustedDate =
    if (today >= supposedMaxLimitDate) {
      // date + 1j if...
