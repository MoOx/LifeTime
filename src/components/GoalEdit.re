...
let make = (~initialGoal: Goal.t, ~onChange, ~onDelete) => {
  ...
  let (title, setTitle) = React.useState(() => initialGoal.title);
  Js.log(("title", title));
  let (type_, setType) =
    React.useState(() => initialGoal.type_->Goal.Type.fromSerialized);
  let (days, setDays) = React.useState(() => initialGoal.days);
  let numberOfDays =
    days->Array.reduce(0., (total, dayOn) => dayOn ? total +. 1. : total);
  let (minutes, setMinutes) =
    React.useState(() => initialGoal.durationPerDay);
  let (categoriesSelected, setCategoriesSelected) =
    React.useState(() => initialGoal.categoriesId);
  let (categoriesOpened, setCategoriesOpen) = React.useState(() => [||]);
  let (activitiesSelected, setActivitiesSelected) =
    React.useState(() => initialGoal.activitiesId);
