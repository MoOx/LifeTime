...
let make = (~onNewGoalPress, ~onEditGoalPress) => {
  ...
  let (forceWelcomeContent, forceWelcomeContent_set) =
        ...
        <View style=Predefined.styles##row>
          {Global.__DEV__
             ? <>
                 <TouchableOpacity
                   onPress={_ =>
                     forceWelcomeContent_set(forceWelcomeContent =>
                       !forceWelcomeContent
                     )
                   }>
         ...
         : settings.goals
           ->Array.map(goal => {
               ...
               let currentActivitiesTime =
                 ...
                 ->Option.getWithDefault(0.);
               let currentTime =
                 currentCategoriesTime +. currentActivitiesTime;
               let numberOfDays =
                 goal.days
               ...
               let durationProgress =
                 ...
                 /. (
                   startDate->Js.Date.getTime
                   -. supposedEndDate->Js.Date.getTime
                 );
               ...
               let backgroundColor =
                 ...
                 ->Array.reduce(Map.String.empty, (map, color) => {
                     map->Map.String.set(
                       color,
                       map->Map.String.get(color)->Option.getWithDefault(0)
                       + 1,
                     )
                   })
                 ->Map.String.toArray
               ...
               let (startColor, endColor) =
                 Goal.Colors.(
                   switch (goal.type_->Goal.Type.fromSerialized) {
                   | Some(_) when !canBeDone && !isAlreadyDone => (
                       danger,
                       bad,
                     )
                   ...
                   | Some(Goal) when progressTonight <= 0.75 => (
                       alert,
                       danger,
                     )
