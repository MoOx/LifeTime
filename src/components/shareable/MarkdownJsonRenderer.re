  ...
  | Element(tag, props, children) =>
    ...
    | "details" =>
      let summary = children->Array.keepMap(child => {
                      switch (child) {
                      | Element(tag, _htmlProps, reasonChildren)
                          when tag == "summary" =>
                        Some(reasonChildren)
                      | _ => None
                      }
      ...
      let filtredChildren =
        children->Array.keepMap(child =>
          switch (child) {
          | Element(tag, _htmlProps, _reasonChildren) when tag == "summary" =>
            None
          | _ => Some(child)
          }
        );
      <Html.Details
        ...
        props
        summary={
          summary->Option.map(summary =>
            renderChildren(~keepNewlines, tag, summary)
          )
        }>
        {renderChildren(~keepNewlines, tag, filtredChildren)}
      </Html.Details>;
    | "div" =>
      <View key> {renderChildren(~keepNewlines, tag, children)} </View>
    | _ =>
...
let required = (body: ReactNative.Packager.required): required => {
  body->Obj.magic;
};
